...
export default function Composer({ inputRef, imageInputRef, message, setMessage, selectedImage, setSelectedImage, chooseImage, imageChatEnabled, isVoiceListening, setIsVoiceListening, isResponding, stopGeneration, handleSend, chatMode, coderTask, setCoderTask, answerStyle, setAnswerStyle, setResponseMode, composerHints, placeholderIndex, hasManualModelChoice, recommendedModelName, selectedModel, models, skills = [] }) {
  ...
  return <form className="ai_match_maker__composer" onSubmit={handleSend}>
    ...
    <div className="ai_match_maker__composer-actions">
      ...
      {imageChatEnabled && <button type="button" onClick={() => imageInputRef.current?.click()} aria-label="Upload an image" title="Upload image"><ImagePlus size={17} /></button>}
      {chatMode === "coder" && <label className="ai_match_maker__model-picker ai_match_maker__coder-task-picker"><select value={coderTask} onChange={(event) => setCoderTask(event.target.value)} aria-label="Choose coding task"><option value="debug">Debug</option><option value="build">Build</option><option value="review">Review</option><option value="refactor">Refactor</option><option value="test">Test</option></select><ChevronDown size={16} aria-hidden="true" /></label>}
      <label className="ai_match_maker__model-picker ai_match_maker__style-picker"><select value={answerStyle} onChange={(event) => { const value = event.target.value; setAnswerStyle(value); setResponseMode(chatMode === "normal" ? value : "standard"); localStorage.setItem("next_ai_response_mode", value); }} aria-label="Answer style">
        {chatMode === "coder" ? <><option value="standard">Standard</option><option value="structured">Structured</option><option value="code-only">Code only</option></> : <><option value="concise">Concise</option><option value="standard">Standard</option><option value="detailed">Detailed</option></>}
      </select><ChevronDown size={16} aria-hidden="true" /></label>
      {isResponding ? <button type="button" className="ai_match_maker__send ai_match_maker__stop" onClick={stopGeneration} aria-label="Stop generating" title="Stop generating"><Square size={13} fill="currentColor" /></button> : <button className="ai_match_maker__send" type="submit" disabled={!message.trim() && !selectedImage} aria-label="Send message"><ArrowUp size={17} /></button>}
    </div>
    {hasManualModelChoice && recommendedModelName && selectedModel && models.find((model) => model.id === selectedModel)?.displayName !== recommendedModelName && <span className="ai_match_maker__model-hint" title="NeXT recommended a model for this prompt; you chose to use another model.">Recommended model: {recommendedModelName} · Your selection: {models.find((model) => model.id === selectedModel)?.displayName || "Selected model"}</span>}
